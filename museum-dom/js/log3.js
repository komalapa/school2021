...
Слайдер в секции Video +14/20
    ...
    при перелистывании слайдов буллет активного слайда подсвечивается (выделяется стилем) +2
    НЕТ если основное видео проигрывалось при перелистывании слайдера, проигрывание видео останавливается, прогресс бар сдвигается к началу, иконки "Play" на панели управления и по центру видео меняются на первоначальные +2
    даже при частых кликах или свайпах нет ситуации, когда слайд после перелистывания находится не по центру, нет ситуации, когда видны одновременно два слайда +2

Кастомный видеоплеер +35/36
    ...
    ЧАСТИЧНО панель управления в полноэкранном режиме визуально выглядит так же, как на макете - кнопки равномерно распределены по всей ширине страницы, относительные размеры между кнопками и ползунками, а также относительные размеры самих кнопок остались прежними +1/2
    Не смогла сделать красиво растянутый интерфейс не меняя длину прогресс баров. Разместила посередине.
...
Дополнительно клавиши с youtube, справка по h, текстовое отображение действий с видео (как в stage0) + выход по Esc 
Клавиши срабатывают только когда видео видно и тогда не работает дефолтное действие кнопок (scroll по пробелу и стрелкам)
...
Калькулятор продажи билетов в форме продажи билетов +12/14
    ...
    НЕТ нельзя выбрать дату в прошлом +2
    когда пользователь выбирает время в форме слева, оно отображается в билете справа +2
    НЕТ время можно выбирать с 9:00 до 18:00 с интервалом в 30 минут +2
...
Валидация формы +14/16
    ...
    валидация номера телефона: номер содержит только цифры; без разделения или с разделением на две или три цифры; разделение цифр может быть через дефис или пробел; с ограничением по количеству цифр не больше 10 цифр +2/4
Правило скорректировала т.к. то что описано не дает ввести стандартное xxx-xxx-xx-xx
...
Красный использован из макета. При неверном значении кнопка блокируется.

Интерактивная карта в секции Contacts +0/12
...
Любой собственный дополнительный функционал, улучшающий качество проекта. Например, ночная тема, плавная смена изображений в блоке Tickets, всплывающее окно с информацией про картины и их авторов, кнопка прокрутки страницы вверх, возможность проголосовать за понравившиеся картины с сохранением данных в local storage, всё зависит от вашей фантазии и чувства вкуса. Для удобства проверки выполненный вами дополнительный функционал включите в самооценку, которую выведите в консоль браузера +10
Дополнила видеоплеер чтобы им можно было пользоваться (выход по esc, горячие клавиши только когда плеер виден)

127/150

Сколько успела. Буду улучшать позже. Замечания приветствуются)
`)