...
const getWinnersList = async (
  ...
  if (!page) {
    ...
    if (+count > carsPerWinnerPage) {
      ...
      const winners: RespWinner[] = await resp2.json();
...
const updateWinner = async (
  ...
  const response: Response = await fetch(`${API_URL}winners/${id}`, {
    method: 'PUT',
    headers: {
...
export {
  getWinner, getWinnersList, addWinner, deleteWinner, updateWinner,
};
